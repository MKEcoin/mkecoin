...
gsigs = 'https://github.com/MKEcoin-project/gitian.sigs.git'
...
def setup():
    ...
    if not os.path.isdir('MKEcoin'):
        subprocess.check_call(['git', 'clone', args.url, 'MKEcoin'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/MKEcoin/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'MKEcoin='+args.commit, '--url', 'MKEcoin='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/MKEcoin-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/MKEcoin/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/MKEcoin/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    os.makedirs('builder/inputs/MKEcoin', exist_ok=True)
    os.chdir('builder/inputs/MKEcoin')
    if args.pull:
