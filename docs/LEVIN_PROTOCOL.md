...
used by MKEcoin. The protocol is largely inherited from cryptonote, but has
undergone some changes.

This document also may differ from the `struct bucket_head2` in MKEcoin's
...
> This document does not currently list all data being sent by the MKEcoin
> protocol, that portion is a work-in-progress. Please take the time to do it
> if interested in learning about MKEcoin p2p traffic!


## Header
This header is sent for every MKEcoin p2p message.
...
MKEcoin when the link is not encrypted. SSL has been proposed as a means to
...
### Command
An unsigned 32-bit little endian integer representing the MKEcoin specific
...
### Requests
Requests are the basis of the admin protocol for MKEcoin. The `Q` bit must be
